...
def read_excel_comments(file_path):
    ...
    if 'comments' in df.columns:
        return df, df['comments'].dropna().astype(str).tolist()  # 返回 df 和评论列表
    else:
        print("Excel 文件中未找到 'comments' 列")
        return df, []
...
if __name__ == "__main__":
     file_path = "D:/python_test/SparkLiteAI_comments/ExcelCom/comments.xlsx"  # 修改为你的 Excel 文件路径
     df, comments_list = read_excel_comments(file_path)

     main(
        appid="your_appid", #传入你的APPId
        api_secret="your_api_secret",   #传入你的API_Secret
        api_key="your_api_key", #传入你的APIKey
        ...
        query="分析以下评论的优点、缺点、退货原因，并用中文输出，使用Markdown表格格式: " + " ".join(comments_list[:10])
